...
BASE_DIR = "data"
SUMMARY_DIR = os.path.join(BASE_DIR, "summary")  # renamed folder
...
def fetch_user_txt_summaries(service, user_id):
    ...
    # Step 1: Find the folder ID for the user
    ...
    # Step 2: List all .txt files in that folder
...
Make it clear that you are an AI assistant, speaking directly to the user — not as an observer of others.
Your responses must be based ONLY on the information provided in the context (this user's cumulative summary).
</TASK>

<STRICT PRIVACY & CONTENT RULES>
NEVER mention, quote, or refer to:
- Any names, people, or participants
- DON'T give names or any personal details
...
if __name__ == "__main__":
    ...
    # Step 1: Fetch .txt summaries
    fetch_user_txt_summaries(service, user_id)

    # Step 2: Generate cumulative summary
    generate_cumulative_summary(user_id)

    # Step 3: Vectorize cumulative summary
    ...
    while True:
        ...
        if question.lower() == "exit":
            break
        answer = get_answer_from_user_db(user_id, question)
        print("\nAssistant:", answer)
