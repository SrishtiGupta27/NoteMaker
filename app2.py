...
# A broad query for common document types
...
def get_user_paths(user_id):
    ...
    # Returning the transcript path instead of the raw path
...
def generate_session_summary(raw_text, original_filename, user_id):
    ...
    # Updated to receive 3 returns from get_user_paths, but only using summary dir
    _, user_summary_dir, _ = get_user_paths(user_id) 
    
    # Use a unique name for the session summary
...
def fetch_and_process_session_files(service, user_id):
    """
    Fetch various document files, convert them to text, and generate session summaries.
    """
    ...
    # UPDATED: Get transcript directory
    ...
    for f in files_to_process:
        ...
        # Check if we have already processed a summary for this file (simplistic check)
        ...
        if mime_type in DOC_MIME_TYPES:
            ...
            # Use files().export for Google native formats
            request = service.files().export_media(fileId=file_id, mimeType=export_mime_type)
            # Adjust file extension for the downloaded content
            download_filename = f"{file_name}.txt" if 'text' in export_mime_type else file_name
            
        elif mime_type == 'text/plain':
            # Use files().get_media for regular TXT files
            request = service.files().get_media(fileId=file_id)
            download_filename = file_name
        else:
            # Fallback for other documents (e.g., PDFs, DOCX) - needs external parsing logic
            print(f"Skipping {file_name}: Unsupported mimeType {mime_type}. Needs external parser.")
            continue
        ...
        # 4. Read content (assuming it's readable text after export/download)
        try:
            with open(local_transcript_path, "r", encoding="utf-8") as raw_f:
                raw_text = raw_f.read()
        except Exception as e:
            print(f"Error reading file {local_transcript_path}: {e}")
            os.remove(local_transcript_path) # Clean up if reading fails
            continue
    ...
    print(f"Processed {len(summary_paths)} new sessions for {user_id}.")
    return summary_paths


def generate_cumulative_summary(user_id):
    # Updated to receive 3 returns from get_user_paths
    _, user_summary_dir, _ = get_user_paths(user_id) 
    ...
    if "service" in st.session_state and user_id:
        if st.button("📥 Fetch & Update Summaries"):
            with st.spinner("Fetching and updating summaries..."):
                try:
                    fetch_and_process_session_files(st.session_state["service"], user_id)
                    
                    generate_cumulative_summary(user_id) 
                    vectorize_cumulative_summary(user_id)
                    
                    st.success("✅ Files processed, summaries updated, and knowledge base vectorized successfully.")
                except Exception as e:
                    st.error(f"❌ Failed: {e}")
    else:
